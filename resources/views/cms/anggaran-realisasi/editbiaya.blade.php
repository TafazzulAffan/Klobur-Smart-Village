...
@section('header')
<h2 class="font-bold text-lg">Edit Data APBDes Pembiayaan</h2>
@endsection

@section('content')
<div class="p-8 shadow-xl">
    <form action="{{ route('update_biaya') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col gap-6 w-3/4 mx-auto">
            <input type="hidden" name="id" value="{{old('id', $anggaranbiaya->id)}}"/>
            <div>
                ...
                <select name="tahun" id="tahun" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green focus:border-green block w-full p-2.5">
                    <option selected value="{{ old('tahun', $anggaranbiaya->tahun) }}">{{$anggaranbiaya->tahun}}</option>
            ...
            <div>
                ...
                <select name="detail_jenis_anggaran" id="detail-anggaran" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green focus:border-green block w-full p-2.5">
                    <option selected value="{{ old('detail_jenis_anggaran', $anggaranbiaya->detail_jenis_anggaran) }}" >{{$anggaranbiaya->detail_jenis_anggaran}}</option>
            ...
            <div>
                ...
                <input type="text"  value="{{ old('nilai_anggaran', $anggaranbiaya->nilai_anggaran) }}"
            ...
            <div>
                ...
                <input type="text" value="{{ old('nilai_realisasi', $anggaranbiaya->nilai_realisasi) }}"
            ...
            <div>  
                ...
                rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-green focus:border-green" >{{$anggaranbiaya->rincian}}</textarea>
            </div>
