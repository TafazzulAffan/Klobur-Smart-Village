...
@section('header')
    <p class="text-sm md:text-base">
        <a href="/"><span class="text-green">Kelola Berita</span></a>
    </p>
    <div class="flex items-center gap-5 justify-end">
        <!-- Search Start -->
        <form class="w-72" action="/kelola-berita" method="GET">
                ...
                <tbody class="text-left">
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4 border-r whitespace-nowrap">
                            <div
                                class="py-1 px-6 w-fit flex gap-2 items-center rounded-full justify-center mx-auto {{ $berita->status == 'Aktif' ? 'bg-green-300 text-green-700' : 'bg-red-300 text-red-700' }}">
                                <div
                                    class="w-2 h-2 rounded-full {{ $berita->status == 'Aktif' ? 'bg-green-700' : 'bg-red-700' }}">
                                </div>
                                <p>{{ $berita->status }}</p>
                            </div>
                        </td>
                        <td class="px-1 py-4 flex justify-center gap-3">
                            <a href="/hapus-berita/{{ $berita->id }}">
                                <img class="bg-red-500 hover:bg-red-700 w-4 h-4 box-content p-2 rounded"
                                    src="{{ asset('icon/delete.svg') }}" alt="">
                            </a>
                            <a href="/edit-berita/{{ $berita->id }}/edit">
